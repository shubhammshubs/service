...
class InProcessCalls extends StatefulWidget {
  final String mobileNumber;

  InProcessCalls({required this.mobileNumber});

  @override
  _HoldCallsState createState() => _HoldCallsState();
}

class _HoldCallsState extends State<InProcessCalls> {
    ...
    if (response.statusCode == 200) {
      ...
      if (dataList.isNotEmpty) {
        setState(() {
          ...
          apiDataList = dataList.where((item) => item['status'] == "In Process").toList();
        });
      } else {
    ...
    void handleUpdate() async {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => InProcessCalls(mobileNumber: widget.mobileNumber),
          ),
        );
